...
Due to limited time, we did not have our real life maze navigation run successfully. We planned to implement an indicator to indicate that the searching is done, but we did not have time to finish this part. However, we are certain that our DFS algorithm works in various situations since we had a random maze generator and tested it many times. 
